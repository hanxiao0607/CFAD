...
def main():
    ...
    gae = GAE.GAE(len(df_train), options['d'], options['x_dim'], options['seed'], options['num_encoder_layers'],
                        ...
                        options['hidden_size'], options['latent_dim'], options['l1_graph_penalty'], options['use_float64'])
    ...
    gaes = GAES_trainer.GAESTrainner(gae.net.encoder, df_train.iloc[:, :-3].values.reshape(-1, options['d'], options['x_dim']), W_est_old,
                        max_epoch=200, n=len(df_train), d=options['d'])
    gaes.train()
    print('Start pretrain AAE')
    train_iter, eval_iter = utils.pretrain_split(df_train, df_eval)
    ...
    df_org = utils.get_pretrain_result(gaes, aae_trainer, df_test, df_test_cf)
    print('Start retrain AAE')
    train_iter, eval_iter = utils.retrain_split(gaes, df_train, df_eval)
    ...
    df_ad = utils.get_retrain_result(gaes, aae_trainer, df_test, df_test_cf)
