...
def get_samples(dataset, upper=None, lower=None):
    ...
    if (upper is None) or (lower is None):
        upper = np.quantile(lst_y, 0.99)
        outer_upper = np.quantile(lst_y, 0.80)
        inner_upper = np.quantile(lst_y, 0.70)
        middle_upper = np.quantile(lst_y, 0.51)
        middle_lower = np.quantile(lst_y, 0.49)
        lower = np.quantile(lst_y, 0.01)
        outer_lower = np.quantile(lst_y, 0.20)
        inner_lower = np.quantile(lst_y, 0.30)
    lst_n_c, lst_n_nc, lst_n_m, lst_ab_c, lst_ab_nc, lst_ab_m = [], [], [], [], [], []
    lst_n_c_cf, lst_n_nc_cf, lst_n_m_cf, lst_ab_c_cf, lst_ab_nc_cf, lst_ab_m_cf = [], [], [], [], [], []
    for i in range(len(df_X)):
        sample_X = df_X.iloc[i]
        sample_X_cf = df_X_cf.iloc[i]
        if sample_X['y'] > inner_upper and sample_X['y'] < outer_upper:
            if sample_X_cf['y'] > outer_lower and sample_X_cf['y'] < inner_lower:
                lst_n_c.append(sample_X.values)
                lst_n_c_cf.append(sample_X_cf.values)
            elif sample_X_cf['y'] > inner_upper and sample_X_cf['y'] < outer_upper:
                lst_n_nc.append(sample_X.values)
                lst_n_nc_cf.append(sample_X_cf.values)
            else:
                pass
            ...
            if sample_X_cf['y'] < lower or sample_X['y'] > upper:
                lst_ab_nc.append(sample_X.values)
                lst_ab_nc_cf.append(sample_X_cf.values)
            elif (sample_X_cf['y'] > middle_lower) and (sample_X_cf['y'] < middle_upper):
                lst_ab_c.append(sample_X.values)
                lst_ab_c_cf.append(sample_X_cf.values)
            else:
                pass
    ...
    df_train = pd.concat([df_n_nc_major.iloc[:10000], df_n_c_major.iloc[:10000], \
                          df_n_nc_minor.iloc[:10000], df_n_c_minor.iloc[:10000]])
    df_train = df_train.sample(n=len(df_train), random_state=42)
    df_eval_set = pd.concat([df_n_nc_major.iloc[0:1], df_n_c_major.iloc[0:1], \
                             df_n_nc_minor.iloc[0:1], df_n_c_minor.iloc[0:1]])
    # df_test_n = pd.concat([df_n_nc_major.iloc[10000:11000], df_n_c_major.iloc[1000:2000], \
    ...
    # df_test_ab = pd.concat([df_ab_nc_major.iloc[:100], df_ab_c_major.iloc[:100], \
    #                         df_ab_nc_minor.iloc[:100], df_ab_c_minor.iloc[:100]])
    df_test_n = pd.concat([df_n_nc_major.iloc[10000:11000], df_n_c_major.iloc[10000:11000], \
                           df_n_nc_minor.iloc[10000:11000], df_n_c_minor.iloc[10000:11000]])
    df_test_ab = pd.concat([df_ab_nc_major.iloc[:100], df_ab_c_major.iloc[:100], \
                            df_ab_nc_minor.iloc[:100], df_ab_c_minor.iloc[:100]])
...
def retrain_split(gaes, df_train, df_eval, scaler, device='cuda:0'):
