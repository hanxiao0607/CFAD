...
from utils import config_utils, utils, synthetic_dataset, adult_config
...
def main():
    ...
    y_score = scaler_minmax.fit_transform(ocsvm.score_samples(X_test).reshape(-1,1)*(-1))
    y_pred_cf = ocsvm.predict(X_test_cf)
    y_pred_cf[y_pred_cf==1] = 0
    y_pred_cf[y_pred_cf==-1] = 1
    y_score_cf = scaler_minmax.fit_transform(ocsvm.score_samples(X_test_cf).reshape(-1,1)*(-1))
    ...
    print(f'OCSVM the prediction changed: {pca_cf / total}')
